...
const CONFIG = {
  ...
  googleOrderScriptUrl: "https://script.google.com/macros/s/AKfycbwRBK3iJPULbTggqDsVNflD0Tn9Gvjib3kw67D0MrtIDbBupxcnd9zbivsC0AaCP69_/exec",
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Синхронизация цветов шапки VK с текущей темой
    try {
      window.vkBridge?.send('VKWebAppSetViewSettings', {
        status_bar_style: isLightTheme ? 'dark' : 'light',
        action_bar_color: bgColor,
        navigation_bar_color: bgColor
      }).catch(() => {});
    } catch(e) {}
  }, [isLightTheme]);
  ...
  // 0. Нативная вибрация ВКонтакте через VK Bridge
  const triggerHaptic = useCallback((type = 'impact', style = 'light') => {
    try {
      if (type === 'impact') {
        window.vkBridge?.send('VKWebAppTapticImpactOccurred', { style: style === 'light' ? 'light' : 'medium' }).catch(() => {});
      } else if (type === 'selection') {
        window.vkBridge?.send('VKWebAppTapticSelectionChanged').catch(() => {});
      } else if (type === 'notification') {
        window.vkBridge?.send('VKWebAppTapticNotificationOccurred', { type: style === 'error' ? 'error' : 'success' }).catch(() => {});
      }
    } catch (e) {
      console.error('VK Haptic error:', e);
    }
  }, []);
  ...
  // Копирование через VK Bridge для обхода ограничений браузера внутри iframe
  const handleCopy = async () => {
    const link = CONFIG.shareLink && CONFIG.shareLink !== "" ? CONFIG.shareLink : window.location.origin;
    try {
      if (window.vkBridge) {
        await window.vkBridge.send("VKWebAppCopyText", { text: link });
      } else {
        throw new Error("No vkBridge");
      }
      setCopied(true);
      triggerHaptic('notification', 'success');
      setTimeout(() => setCopied(false), 2000);
    } catch (error) {
      // Фолбэк на обычное копирование, если запущено вне VK
      try {
